...
static LogicalResult rewriteInsertSlice(OpBuilder &builder,
  ...
  Value workspace =
      createAllocWorkspace(builder, loc, getMemRefType(destType),
                           getDynamicTensorDims(builder, loc, insertOp.getDest(),
                                                destType));
  if (!insertOp.getDest().getDefiningOp<tensor::EmptyOp>())
    materializeInDestination(builder, loc, insertOp.getDest(), workspace);
...
rewriteVVCommInsertSlice(OpBuilder &builder,
  ...
  for (auto extractOp : pairedExtracts) {
    ...
    auto localType = getMemRefType(extractResultType);
    Value local = builder.create<memref::AllocOp>(

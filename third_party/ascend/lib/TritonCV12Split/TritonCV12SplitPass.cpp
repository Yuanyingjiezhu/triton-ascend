...
static void reportCV12SplitFailure(ModuleOp moduleOp, StringRef passName) {
  llvm::errs() << "[TritonCV12Split] failed in " << passName
               << " pass for CV1:2 split for module "
...
struct TritonCV12SplitPass
  ...
  void runOnOperation() override {
    ...
    if (failed(runSingleModulePass(
            moduleOp, triton::createNormalizeDotPass()))) {
      LDBG("TritonNormalizeDot failed, rolling back");
      reportCV12SplitFailure(moduleOp, "NormalizeDot");
      restoreModule(moduleOp, *backupModuleOp);
      return;
    }

    if (failed(runSingleModulePass(
            moduleOp, triton::createSeparateMetaDataUsePass()))) {
      LDBG("TritonSeparateMetaDataUse failed, rolling back");
      reportCV12SplitFailure(moduleOp, "SeparateMetaDataUse");
      restoreModule(moduleOp, *backupModuleOp);
      return;
    }
    ...
    if (failed(runSingleModulePass(moduleOp, triton::createStartSlicePass()))) {
      ...
      reportCV12SplitFailure(moduleOp, "StartSlice");
      restoreModule(moduleOp, *backupModuleOp);
      return;
    }
    ...
    if (failed(runSingleModulePass(
      ...
      reportCV12SplitFailure(moduleOp, "BubbleUpExtractSlice");
      restoreModule(moduleOp, *backupModuleOp);
      return;
    }

    if (failed(runSingleModulePass(moduleOp,
                                   triton::createRemoveUseTagsPass()))) {
      LDBG("TritonRemoveUseTags failed, rolling back");
      reportCV12SplitFailure(moduleOp, "RemoveUseTags");
      restoreModule(moduleOp, *backupModuleOp);
      return;
    }
    ...
    if (failed(runSingleModulePass(
      ...
      reportCV12SplitFailure(moduleOp, "RewriteCommunicationSlice");
      restoreModule(moduleOp, *backupModuleOp);
      return;
    }
    ...
    if (hasResidualSliceTags(moduleOp)) {
      ...
      reportCV12SplitFailure(moduleOp, "ResidualSliceTagCheck");
      restoreModule(moduleOp, *backupModuleOp);
      return;
    }
